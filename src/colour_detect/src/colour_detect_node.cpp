#include "colour_detect_node.h"

// Dynamic tuning of color filter
void colourFinder::configCallback(const colour_detect::colourParamsConfig &config, uint32_t level)
{
      ...
      height = config.colour_height_treshold;
}

// Setting message values to a default
void colourFinder::init_msg(cv_bridge::CvImagePtr cv_ptr) 
{
  ...
  detected.pos_x = -1;
  detected.pos_y = -1;
  detected.colours_leaving_image = 0;
}

 // Red filter, blur and egde detection
void colourFinder::redFilterAndEgde(cv_bridge::CvImagePtr cv_ptr) 
{
...
void colourFinder::Contours(cv_bridge::CvImagePtr cv_ptr) 
{
...
void colourFinder::findDistance(cv_bridge::CvImagePtr cv_ptr) 
{
    ...
    if (detected.pos_x > (detected.frame_width/2 - 200) && detected.pos_x < (detected.frame_width/2 + 200)) {
        if  (detected.pos_y > (detected.frame_height/2 - 200) && detected.pos_y < (detected.frame_height/2 + 200)) {
            ...
            detected.distance_to_colour = distance;
...
void colourFinder::drawOnImage(cv_bridge::CvImagePtr cv_ptr) 
{
...
// opencb callback
void colourFinder::run(const sensor_msgs::ImageConstPtr& msg)
// Reading the image to cv_ptr
{
  ...
  // Dynamic reconfigure
  f = boost::bind(&colourFinder::configCallback, this, _1, _2);
